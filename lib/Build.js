...
class Build {
      ...
      for (cursor.z = this.min.z; cursor.z < this.max.z; cursor.z++) {
        for (cursor.x = this.min.x; cursor.x < this.max.x; cursor.x++) {
          ...
          if (stateInWorld !== wantedState) {
            if (wantedState === 0) {
              if (this.breakNoneAir) this.actions.push({ type: 'dig', pos: cursor.clone() })
              continue
            }
            if (this.breakBlockDifferent && stateInWorld !== 0) {
  ...
  getAvailableActions () {
    return this.actions.filter(action => {
      ...
      if (action.dependsOn) {
        return !this.actions.find(a => a.pos.x === action.dependsOn.x && a.pos.y === action.dependsOn.y && a.pos.z === action.dependsOn.z)
      }
